...
MultipartUpload.prototype.initialise = function() {
  ...
  var self = this;
  self.logger.log('Pinging: https://api.kloudless.com/v0/accounts/' + self.account_id + '/multipart');
...
MultipartUpload.prototype.loadSession = function() {
  ...
  var self = this;
  self.logger.log('Pinging: https://api.kloudless.com/v0/accounts/multipart/' + self.session_id);

  return request({
    url: 'https://api.kloudless.com/v0/accounts/multipart' + self.session_id,
    headers: {
      'Authorization': 'ApiKey ' + API_KEY,
      'Content-Type': 'application/json'
    },
