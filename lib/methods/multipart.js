...
MultipartUpload.prototype.initialise = function() {
  ...
  var self = this;
  self.logger.log('Pinging: ' + self._kloudless.getApiEndpoint() + '/accounts/' + self.account_id + '/storage/multipart' + (self.overwrite? '?overwrite=True':''));
  ...
  return request({
    url: self._kloudless.getApiEndpoint() + '/accounts/' + self.account_id + '/storage/multipart' + (self.overwrite? '?overwrite=True':''),
    headers: {
...
MultipartUpload.prototype.loadSession = function() {
  ...
  var self = this;
  self.logger.log('Pinging: ' + self._kloudless.getApiEndpoint() + '/accounts/' + self.account_id + '/storage/multipart/' + self.session_id);

  return request({
    url: self._kloudless.getApiEndpoint() + '/accounts/' + self.account_id + '/storage/multipart/' + self.session_id,
    headers: {
      'Authorization': 'ApiKey ' + API_KEY,
      'Content-Type': 'application/json'
    },
...
MultipartUpload.prototype.upload = function() {
  ...
  return new Promise(function(resolve) {
    ...
    var uploadWorker = function(data, callback) {
      connections[data.part] = (function(c, p) {
        return self.uploadChunk(c, p)
          .then(function() {
            ...
            if (Object.keys(connections).length < self.max_connections) {
              // self.logger.log("Resuming data loading:", connections);
              self.file.resume();
            }
            callback();
          });
      })(data.chunk, data.part);
    }

    uploadQueue = async.queue(uploadWorker, self.max_connections);
    ...
    self.file.on('data', function(datum) {
      ...
      while (loadedData.length >= self.chunk_size) {
        ...
        uploadQueue.push({chunk: chunk, part: part}, function (){})
      ...
      if (Object.keys(connections).length >= self.max_connections) {
        // self.logger.log("Pausing data loading:", connections);
        self.file.pause();
      }
    });
    ...
    self.file.on('end', function() {
      ...
      if (loadedData.length > 0) {
        ...
        uploadQueue.push({chunk: loadedData, part: part}, complete)
...
MultipartUpload.prototype.finalise = function() {
  ...
  var self = this;
  return request({
    url: self._kloudless.getApiEndpoint() + '/accounts/' + self.account_id + '/storage/multipart/' + self.session_id + '/complete',
    headers: {
      'Authorization': 'ApiKey ' + API_KEY,
      'Content-Type': 'application/json'
    },
...
MultipartUpload.stopSession = function(options) {
  ...
  var self = this;
  return request({
    url: self._kloudless.getApiEndpoint() + '/accounts/' + account_id + '/storage/multipart/' + session_id,
    headers: {
      'Authorization': 'ApiKey ' + API_KEY
    },
    agentOptions: self.tls_agent_options,
